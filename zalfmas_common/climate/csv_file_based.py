...
class TimeSeries(
    ...
    def __init__(
        ...
        self._pandas_csv_config = {
            **self._pandas_csv_config_defaults,
            **pandas_csv_config,
        }
    ...
    async def subrange(
        ...
        _context.results.timeSeries = TimeSeries.from_dataframe(
            sub_df,
            metadata=self._meta,
...
class Dataset(climate_capnp.Dataset.Server, common.Identifiable, common.Persistable):
    ...
    def location_at(self, row: int, col: int, coord=None, timeseries=None):
        if not self._cache_data or (row, col) not in self._locations:
            if not coord:
            ...
            loc = climate_capnp.Location.new_message(
                ...
                customData=[
                    climate_capnp.Location.KV.new_message(
                        key="row/col",
                        value=geo_capnp.RowCol.new_message(row=row, col=col),
                    )
                ],
            )
            if timeseries:
    ...
    async def streamLocations_context(
        self, context
        ...
        if loc_id and len(loc_id) > 0:
            it = itertools.dropwhile(
                lambda rcll: self.create_location_id(*rcll[0]) != loc_id,
